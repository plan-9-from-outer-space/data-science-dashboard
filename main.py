...
st.title(":robot_face: Population of Canada")
...
def format_date_for_comparison(date):
    """ Allows comparison of two string dates: 
          ['Q3 1991' < 'Q2 2001'] becomes [1991.75 < 2001.25] """
    if date[1] == 2:
...
def display_dashboard(start_date, end_date, target):
    ...
    with tab1:
        ...
        with col2:
            ...
            fig, ax = plt.subplots()
            ax.set_title(f'Population Change of {target}')
    ...
    with tab2:
        ...
        all_targets = st.multiselect("Choose other locations", \
            options=filtered_df.columns[1:], default=[target])
        
        fig, ax = plt.subplots()
        ax.set_title('Population Change Comparison')
        for location in all_targets:
            ax.plot(filtered_df['Quarter'], filtered_df[location], label=location)
        ax.set_xlabel('Time')
        ax.set_ylabel('Population')
        ax.set_xticks([filtered_df['Quarter'].iloc[0], filtered_df['Quarter'].iloc[-1]])
        ax.legend(loc='best')
...
if start_date not in df['Quarter'].tolist() or end_date not in df['Quarter'].tolist():
    ...
    st.error("Bad date selection - Start date must come before end date.")
else:
